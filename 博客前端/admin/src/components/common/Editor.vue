  ...
  <div class="editor">
    ...
    <div class="operate-bar">
      ...
      <section class="btn-container">
        <button id="delete" class="delete" @click="deleteArticle">删除文件</button>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters(["getTags"]),
      // 因为这个title是数据双向绑定，因此它可能会被改变，如果我们直接从mapState中读取它的话
    ...
    mounted() {
      ...
      // 将vuex里面的正在编辑的文章的相关信息输出到编译器里面
      this.simplemde.value(this.content);

      // 绑定编辑器的按键时间以及复制，粘贴的事件发生
    ...
    methods: {
      ...
      autoSave: debounce(function() {
        if (this.id) {
          this.$store.dispatch("saveActicle", {
            title: this.title,
            id: this.id,
      ...
      addTag(){
        ...
        // 只是一个单纯的切换功能,第一次点击+的时候,显示input表单,第二次在input中输入内容按下enter就隐藏表单
